...
> **Note**: Internally functions receive a hierarchical *environment* frame reference, which is decently optimized, where every frame contains a ?context local that includes a `DecimalContext` reference that may be a null pointer (which means *skip to parent frame*).
>
